...
public class JTRegParser {
    ...
    public static class TestSuite {
        ...
        public void toAntJUnitXML(OutputStream os) throws IOException {
            ...
            rootElement.setAttribute(ATTR_NAME, name == null ? UNKNOWN : removeNonprintableCharacters(name));
            rootElement.setAttribute(TIMESTAMP, removeNonprintableCharacters(TimeUtils.toISO8601(date)));
            rootElement.setAttribute(HOSTNAME, hostname == null ? UNKNOWN : removeNonprintableCharacters(hostname));
            final Element propsElement = new Element(PROPERTIES);
            rootElement.addContent(propsElement);
            if (props != null) {
                final Enumeration<? extends Object> e = props.propertyNames();
                while (e.hasMoreElements()) {
                    final String name = (String) e.nextElement();
                    final Element propElement = new Element(PROPERTY);
                    propElement.setAttribute(ATTR_NAME, removeNonprintableCharacters(name));
                    propElement.setAttribute(ATTR_VALUE, removeNonprintableCharacters(props.getProperty(name)));
                    propsElement.addContent(propElement);
                }
            }
            for (TestResult result : results) {
                result.appendTo(rootElement, f);
            }
            if (stdout.length() > 0) {
                final Element nested = new Element(SYSTEM_OUT);
                rootElement.addContent(nested);
                nested.addContent(removeNonprintableCharacters(stdout.toString()));
            }
            if (stderr.length() > 0) {
                final Element nested = new Element(SYSTEM_ERR);
                rootElement.addContent(nested);
                nested.addContent(removeNonprintableCharacters(stderr.toString()));
            }
            rootElement.setAttribute(ATTR_TESTS, "" + numTests);
            rootElement.setAttribute(ATTR_FAILURES, "" + numFailed);
            rootElement.setAttribute(ATTR_ERRORS, "" + numErrors);
            rootElement.setAttribute(ATTR_SKIPPED, "" + numSkipped);
            rootElement.setAttribute(ATTR_TIME, String.format(Locale.ROOT, "%.3f", seconds));

            Document doc = new Document(rootElement);

            new XMLOutputter(f).output(doc, os);
        }
    ...
    public static class TestResult {
        ...
        public void appendTo(Element suite, Format f) {
            Element testElement = new Element(TESTCASE);
            testElement.setAttribute(ATTR_NAME, name == null ? UNKNOWN : removeNonprintableCharacters(name));
            testElement.setAttribute(ATTR_CLASSNAME, removeNonprintableCharacters(classname));
            testElement.setAttribute(ATTR_TIME, String.format(Locale.ROOT, "%.3f", seconds));
            suite.addContent(testElement);
            if (Status.SKIPPED.equals(status)) {
                final Element nested = new Element(ATTR_SKIPPED);
                if (message != null) {
                    nested.setAttribute(ATTR_MESSAGE, removeNonprintableCharacters(message));
                }
                testElement.addContent(nested);
            }
            if (Status.FAILED.equals(status) || Status.ERROR.equals(status)) {
                final Element nested = new Element(Status.FAILED.equals(status) ? FAILURE : ERROR);
                if (message != null) {
                    nested.setAttribute(ATTR_MESSAGE, removeNonprintableCharacters(message));
                }
                nested.setAttribute(ATTR_TYPE, type == null ? UNKNOWN : removeNonprintableCharacters(type));
                if (strace != null && strace.length() > 0) {
                    nested.addContent(removeNonprintableCharacters(strace));
                }
                testElement.addContent(nested);
            }
            if (stdout != null && stdout.length() > 0) {
                final Element nested = new Element(SYSTEM_OUT);
                nested.addContent(removeNonprintableCharacters(stdout));
                testElement.addContent(nested);
            }
            if (stderr != null && stderr.length() > 0) {
                final Element nested = new Element(SYSTEM_ERR);
                nested.addContent(removeNonprintableCharacters(stderr));
                testElement.addContent(nested);
            }
        }
    }
